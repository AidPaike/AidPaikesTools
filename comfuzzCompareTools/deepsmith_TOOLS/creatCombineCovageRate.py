...
def createCovageRateFile(start, database_len, filename_tmp):
    for i in tqdm(range(start, database_len), position=1, desc="createCoverFile", leave=False, ncols=180):
        ...
        timeout = "30"
        # 覆盖率文件路径
        LLVM_PROFILE_FILE = f"/root/deepsmith/cov_files/profraws/{i + 1}.profraw"
...
if __name__ == '__main__':
    ...
    start = 0
    # 生成profraws的文件路径
    create_profraws_path = "/root/deepsmith/cov_files/profraws/"
    # 插桩后的chakra编译器路径
    COV_ENGHINES_PATH = '/root/.jsvu/engines/chakra-1.13-cov/ch'
    # 生成的testcase存储路径
    testcase_path = "/root/deepsmith/deepsmith_generate/"
